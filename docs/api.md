...
| GET | `/settings/workspace-roots` | `{ roots: [{ id, path, label, available, unavailable_reason? }], environment: "native"\|"docker" }`. Browse roots for the workspace folder picker. Does not require `repo_root`. `Cache-Control: no-store`. |
