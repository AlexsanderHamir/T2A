...
| GET | `/settings/workspace-roots` | `{ roots: [{ id, path, label, category?, available, unavailable_reason? }], environment: "native"\|"docker" }`. Browse roots for the workspace folder picker. When git repositories are registered, returns those with `category: "registered"`. When none are registered and `HAMIX_BROWSE_ROOTS` is unset, returns OS bootstrap entry points (`install`, `home`, user folders). `category` is one of `registered`, `install`, `home`, `documents`, `desktop`, `downloads`, `pictures`, `music`, `videos`, or `custom`. Does not require `repo_root`. `Cache-Control: no-store`. |
