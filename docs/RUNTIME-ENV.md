...
**Implementation:** `taskapi`-only parsing for listen host, JSON log level / minimized logging, agent queue cap, and dev SSE ticker interval is in [`internal/taskapiconfig`](../internal/taskapiconfig) (`go doc`). The ready-task reconcile tick is fixed in [`pkgs/agents`](../pkgs/agents) (`ReconcileTickInterval`). The **`With*`** middleware stack around `handler.NewHandler` is composed in [`pkgs/tasks/middleware`](../pkgs/tasks/middleware) (`Stack` in `stack.go`) and applied from [`internal/taskapi`](../internal/taskapi) (`NewHTTPHandler`, passing `calltrace.Path` for access logs). Rate limits, idempotency caps, and other HTTP knobs are read from env inside `pkgs/tasks/middleware` when those middlewares initialize.
...
5. `store.NewStore`, **`(*store.Store).SetReadyTaskNotifier`** to the in-process agent queue (defaults in env table), `handler.NewSSEHub`, then the agent worker supervisor (`cmd/taskapi/run_agentworker.go`) reads `app_settings` (singleton row, see [SETTINGS.md](./SETTINGS.md)), and `internal/taskapi.NewHTTPHandler` wires `handler.NewHandler` with a settings-backed `RepoProvider` so `/repo/*` and `@`-mention validation follow `app_settings.repo_root` live. A background **`agents.RunReconcileLoop`** goroutine reconciles **all ready** tasks from the database against the in-memory pending set. `middleware.Stack` adds recovery, metrics, access logging, rate limiting, optional auth, timeouts, body cap, and idempotency (see [`pkgs/tasks/handler/README.md`](../pkgs/tasks/handler/README.md)).
...
> The agent worker (enable/disable, runner choice, cursor binary path, per-run timeout) and the workspace repo (`/repo/*`, `@`-mention root) are **no longer driven by environment variables**. They live in the singleton `app_settings` row and are managed from the SPA Settings page (`/settings`) or `PATCH /settings`. See [SETTINGS.md](./SETTINGS.md) and [AGENT-WORKER.md](./AGENT-WORKER.md). The historical env vars (`T2A_AGENT_WORKER_*`, `REPO_ROOT`) are silently ignored if still present in `.env`.

`dbcheck` uses the same `.env` discovery for `DATABASE_URL` only; it does not consult `app_settings`.
