...
| **M4** | Extract `internal/taskapi` (Option A) OR defer if M3 suffices | Medium–High — **slice 1 done** (`internal/taskapi.NewHTTPHandler` moved out of `cmd/taskapi`). **Slice 2 (partial):** `pkgs/tasks/middleware` (`Stack` + `With*`; `internal/taskapi` calls `middleware.Stack(..., calltrace.Path)`; `handler/middleware_shim.go` re-exports `With*` for tests). **Slice 3 (partial):** `pkgs/tasks/logctx`. **Slice 4 (partial):** `pkgs/tasks/apijson` (`ApplySecurityHeaders`, `WriteJSONError`). A dedicated `handler/middleware` subpackage remains **deferred** (e.g. `writeJSON` + domain error mapping still in `handler`). |
