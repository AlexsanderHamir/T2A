...
| **M4** | Extract `internal/taskapi` (Option A) OR defer if M3 suffices | Medium–High — **slice 1 done** (`internal/taskapi.NewHTTPHandler` moved out of `cmd/taskapi`). **Slice 2 (partial):** `handler.MiddlewareStack` in `handler/stack.go`. **Slice 3 (partial):** `pkgs/tasks/logctx` holds request id + `log_seq` + slog wrappers (no `handler` import); a dedicated `handler/middleware` subpackage remains **deferred** (JSON error helpers still in `handler`). |
