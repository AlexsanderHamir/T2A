...
This document defines **how we measure** logging and correlation in `taskapi`, and **how we increase** test coverage when we change the API or background behavior. It complements [RUNTIME-ENV.md](./RUNTIME-ENV.md) (startup, env), [API-HTTP.md](./API-HTTP.md) (routes, health), and the [DESIGN.md](./DESIGN.md) hub, plus `.cursor/rules/04-structured-logging.mdc` (log shape and safety).
