...
| **Readiness** | Tell DB probe timeouts from other failures | **`GET /health/ready`**: when the **`database`** check fails, **`readiness check failed`** at **Warn** (**`operation`** **`health.ready`**) includes **`timeout_sec`** (**`store.DefaultReadyTimeout`**, **2**) and **`deadline_exceeded`** when the error chain is **`context.DeadlineExceeded`**. See **`docs/DESIGN.md`**. |
