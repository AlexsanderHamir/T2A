...
| **Startup config** | Confirm DB pool, slow-SQL threshold, and HTTP bounds (no secrets) | After migrate, **`taskapi`** logs **`taskapi.db_config`** (pool caps from **`pkgs/tasks/postgres`**, effective **`gorm_slow_query_ms`**) and **`taskapi.http_limits`** (`ReadHeaderTimeout` / `ReadTimeout` / `IdleTimeout` / **`MaxHeaderBytes`**, **`shutdown_timeout_sec`**). Never logs **`DATABASE_URL`**. |
