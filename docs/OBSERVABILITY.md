...
| **Graceful shutdown** | Correlate SIGINT/SIGTERM with HTTP drain | **`shutdown signal received`** (`operation` **`taskapi.shutdown`**, **`signal`** string); after successful **`Server.Shutdown`**, **`http server drained`** (`phase` **`http_done`**). Failures still log **`taskapi.shutdown`** with **`err`**. |
