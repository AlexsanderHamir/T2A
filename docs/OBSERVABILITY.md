...
| **Startup config** | Confirm DB pool, slow-SQL threshold, HTTP bounds, and idempotency (no secrets) | After migrate, **`taskapi`** logs **`taskapi.db_config`** (pool caps from **`pkgs/tasks/postgres`**, effective **`gorm_slow_query_ms`**) and **`taskapi.http_limits`** (`ReadHeaderTimeout` / `ReadTimeout` / `IdleTimeout` / **`MaxHeaderBytes`**, **`shutdown_timeout_sec`**). Before the handler stack, **`taskapi.idempotency`** logs **`enabled`** and **`ttl_sec`** (from **`T2A_IDEMPOTENCY_TTL`**; **`ttl_sec`** 0 when caching is off). Never logs **`DATABASE_URL`**. |
