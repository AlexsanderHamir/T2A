...
- A **tiny allowlist** in `cmd/funclogmeasure` excludes a few **pure, hot-path** helpers from the slog requirement (today: `internal/version.String` for health JSON; `pkgs/repo.isMentionDelimiter` because it runs in a tight loop inside `ParseFileMentions`, which already logs at entry).
