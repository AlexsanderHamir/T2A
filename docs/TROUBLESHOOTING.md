...
**Fix:** For synthetic activity in dev, set **`T2A_SSE_TEST=1`** in **`.env`** and restart **`taskapi`** (see **`docs/DESIGN.md`**). Ensure React Query refetches (the app invalidates on SSE **`onmessage`**).
...
**Fix:** Default API is **`http://127.0.0.1:8080`**. If you change the API port, set **`VITE_TASKAPI_ORIGIN`** for the Vite dev server (and **`DEV_TASKAPI_PORT`** for **`scripts/dev.*`** if you use them). See root **`README.md`**.
