...
1. **Re-run without cache:** `go test ./... -count=1` from the repo root (matches CI).
2. **Flaky or env-related Go tests:** Do not use `t.Parallel()` together with `t.Setenv` / `t.Chdir` in the same test (see `.cursor/rules/BACKEND_AUTOMATION/go-testing-recipes.mdc`). Split or serialize those tests.
...
4. **Web failures:** From `web/`, run `npm ci` then `npm test -- --run` then `npm run lint` then `npm run check:standards` then `npm run build`. Clear stale installs if versions drift (`rm -rf node_modules` on Unix, or delete `web/node_modules` on Windows, then `npm ci`).
