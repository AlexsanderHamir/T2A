...
| Toggle checklist item | `PATCH /tasks/{id}/checklist/items/{itemId}` | Body `{"done": true|false}`. **Requires `X-Actor: agent`.** Human clients default to `user` and receive `400` — only an agent integration may mark items done or undone. Item must belong to the definition source for that task. `200` returns full `{ "items": [...] }`. `X-Actor` stored on audit.                                                                                                                                                                                                                                                           |
