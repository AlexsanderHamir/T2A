...
How T2A runs a task with two agents. Execute implements work; verify judges your checklist. This article explains what your criteria mean in practice.
...
While a cycle runs, agents write short JSON reports to a **worker scratch folder** outside your git repo (`T2A_WORKER_REPORT_DIR`). You do not create, edit, or open these files.
...
| [configuration.md](./configuration.md) | Verify retries, runners, `T2A_WORKER_REPORT_DIR` |
