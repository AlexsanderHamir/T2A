...
- [x] **C1 — Domain metrics:** `taskapi_domain_tasks_created_total`, `taskapi_domain_tasks_updated_total` (`pkgs/tasks/handler/domain_metrics.go`); `taskapi_http_idempotency_cache_evictions_total` (`pkgs/tasks/middleware`); `taskapi_agent_queue_depth` / `taskapi_agent_queue_capacity` (`internal/taskapi/agent_queue_metrics.go`, `cmd/taskapi/run.go`); documented in [API-HTTP.md](./API-HTTP.md).
