...
- [x] **C2 — Store latency:** `taskapi_store_operation_duration_seconds` (`op` = fixed store entrypoint names); implementation [`pkgs/tasks/store/store_metrics.go`](../pkgs/tasks/store/store_metrics.go). Metric contract: [API-HTTP.md](./API-HTTP.md) (Prometheus / `GET /metrics`).
