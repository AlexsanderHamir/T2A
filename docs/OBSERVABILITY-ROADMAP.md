...
- [x] **C2 — Store latency:** `taskapi_store_operation_duration_seconds` histogram (`op` label from a fixed set of store entrypoint names) in [`pkgs/tasks/store/store_metrics.go`](../pkgs/tasks/store/store_metrics.go); documented in [API-HTTP.md](./API-HTTP.md) and [OBSERVABILITY.md](./OBSERVABILITY.md).
