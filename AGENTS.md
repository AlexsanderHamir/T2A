...
| Agent harness | `pkgs/agents/harness/` | Cycle choreography around `runner.Run`: execute/verify phase loop, criteria injection, verification pipeline, git integrity, crash/shutdown recovery, `RunWithRetry` for operator fresh/resume. Called by the worker after admission. See [docs/domain/harness.md](docs/domain/harness.md) and [ADR-0005](docs/adr/ADR-0005-extract-agent-harness.md). |
| Cycle commit tracking | `pkgs/agents/harness/git_commits.go`, `pkgs/tasks/store/internal/commits/` | Observe-first ingest (ADR-0016), commit **status** on rows, verify reads eligible only, `GET /tasks/{id}/commits`, continuation bundle for cross-cycle resume. Deep dives: [cycle-commits.md](docs/domain/cycle-commits.md), [commit-eligibility.md](docs/domain/commit-eligibility.md), [resume-continuation.md](docs/domain/resume-continuation.md), [ADR-0014](docs/adr/ADR-0014-cycle-commit-tracking.md), [ADR-0016](docs/adr/ADR-0016-observe-vs-admit-commits.md). |
