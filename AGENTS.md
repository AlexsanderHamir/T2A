...
| Runner adapter kit | `pkgs/agents/runner/adapterkit/` | Shared CLI adapter mechanics: exec/stream execution, env allow/deny policy, redaction, UTF-8-safe diagnostics, and bounded probes. Concrete runners compose these primitives rather than duplicating them. |
| Cursor CLI runner adapter | `pkgs/agents/runner/cursor/` | V1 `runner.Runner` implementation: `cursor --print --output-format stream-json`, env allowlist (curated safe parent keys plus `T2A_*` + `DATABASE_URL` denied), secret redaction (`Authorization`, cookies, `T2A_*=`, home-path scrub), live progress normalization, and `Probe(cursor --version)` used at startup. Contract: `docs/AGENT-WORKER.md`. |
