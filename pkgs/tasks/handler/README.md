...
The returned `http.Handler` from `NewHandler` is the **inner mux** (routes only). `cmd/taskapi` wraps it with middleware; see **`cmd/taskapi/run.go`** for the current outer chain (`WithRecovery` → … → `NewHandler`).
