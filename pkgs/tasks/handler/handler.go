...
func NewHandler(s *store.Store, hub *SSEHub, rep *repo.Root, opts ...HandlerOption) http.Handler {
	...
	m.Handle("GET /automations", http.HandlerFunc(h.listAutomations))
	m.Handle("POST /automations", http.HandlerFunc(h.createAutomation))
	m.Handle("GET /automations/{id}", http.HandlerFunc(h.getAutomation))
	m.Handle("PATCH /automations/{id}", http.HandlerFunc(h.patchAutomation))
	m.Handle("DELETE /automations/{id}", http.HandlerFunc(h.deleteAutomation))
