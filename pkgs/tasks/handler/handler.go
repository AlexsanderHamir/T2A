...
func NewHandler(s *store.Store, hub *SSEHub, rep *repo.Root, opts ...HandlerOption) http.Handler {
	...
	m.Handle("GET /git/worktrees/{worktreeId}/branches", http.HandlerFunc(h.listWorktreeBranchAssociations))
	m.Handle("POST /git/worktrees/{worktreeId}/branches", http.HandlerFunc(h.associateWorktreeBranch))
	m.Handle("DELETE /git/worktrees/{worktreeId}/branches/{branchId}", http.HandlerFunc(h.removeWorktreeBranchAssociation))
