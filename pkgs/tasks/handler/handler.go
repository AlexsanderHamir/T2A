...
func NewHandler(s *store.Store, hub *SSEHub, rep *repo.Root, opts ...HandlerOption) http.Handler {
	...
	m.Handle("GET /projects/{id}/steps", http.HandlerFunc(h.listProjectSteps))
	m.Handle("POST /projects/{id}/steps", http.HandlerFunc(h.createProjectStep))
	m.Handle("GET /projects/{id}/steps/{stepId}", http.HandlerFunc(h.getProjectStep))
	m.Handle("PATCH /projects/{id}/steps/{stepId}", http.HandlerFunc(h.patchProjectStep))
	m.Handle("DELETE /projects/{id}/steps/{stepId}", http.HandlerFunc(h.deleteProjectStep))
